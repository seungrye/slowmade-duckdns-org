...
| 파일 | 환경변수 |
|---|---|
| `webapp/src/lib/achievements.tsx` | `ACHIEVEMENT_*_POINTS` 18개 이상 |
| `webapp/src/components/post-actions.tsx` | `NEXT_PUBLIC_DELETE_POST_COST` |
| `webapp/src/app/api/post/route.tsx` | `DELETE_POST_COST` |
| `webapp/src/app/api/comments/route.tsx` | `POINTS_FOR_NEW_COMMENT` |
| `webapp/src/app/api/submit/route.tsx` | `POINTS_FOR_NEW_POST` |

- 서버/클라이언트에서 같은 값을 다른 이름으로 참조 (`DELETE_POST_COST` vs `NEXT_PUBLIC_DELETE_POST_COST`)
- 기본값이 파일마다 다를 경우 불일치 버그 발생 가능

**개선**: `webapp/src/lib/env.ts` 하나에 모든 환경변수 파싱/검증 집중
...
**개선**: `webapp/src/lib/require-auth.ts` 헬퍼로 추출
...
**개선**: `webapp/src/lib/api-client.ts` 생성 — `apiGet<T>`, `apiPost<T>`, `apiDelete<T>`
...
**개선**: `webapp/src/lib/show-achievement-toast.ts` 유틸 함수로 추출
...
- 파일: `webapp/src/app/post/view/[[...id]]/like.section.tsx`
...
- `webapp/src/app/api/upload/route.tsx` — `if (!process.env.X) throw new Error(...)` 로 교체
- `webapp/src/app/api/post/route.tsx` — `console.assert` → 400 응답으로 교체

### M3. `!=` 대신 `!==` 사용 ✅

- `webapp/src/app/api/submit/route.tsx` — `!==` 로 교체

### M4. `escapeRegex` 함수 중복 정의 ✅

- `webapp/src/lib/utils.ts` 로 통합, `posts.tsx` / `tags/route.ts` 에서 import로 교체
...
| 영역 | 파일 |
|---|---|
| 업적 시스템 | `webapp/src/lib/achievements.tsx` |
| API 라우트 전체 | `webapp/src/app/api/*/route.tsx` |
| 무한 스크롤 페이지네이션 | `webapp/src/app/infinite-post.section.tsx` |
...
### High
- [ ] D1. `webapp/src/lib/env.ts` — 환경변수 중앙화
- [ ] D2. `webapp/src/lib/require-auth.ts` — API 인증 헬퍼 추출
