...
- [ ] body `{ ids: string[] }` 를 받는다
- [ ] `ids` 가 배열이 아니거나 비어있으면 `400`
- [ ] 대상 item 들의 `_id` 를 조회해 `ItemRevision` 을 cascade 삭제
  (`itemId: { $in: [...] }`)
- [ ] `Item.deleteMany({ id: { $in: ids } })` 로 일괄 삭제
- [ ] `{ deleted: <삭제된 개수> }` 반환 (apiSuccess)

### 카탈로그 UI (`src/app/quests/items/page.tsx`)

- [ ] 각 item 행에 선택 체크박스 추가
- [ ] 목록 상단에 "전체 선택"(현재 보이는 항목 기준) 체크박스 — 일부만
  선택 시 indeterminate 표시
- [ ] "선택 삭제 (N)" 버튼 — 선택 0개면 disabled, 빨강 스타일
- [ ] 클릭 시 `confirm` → `POST /api/quests/items/bulk-delete` →
  선택 초기화 + 목록 재로드
- [ ] 재로드(`load()`) 시 선택 상태 초기화 (stale 선택 방지)
- [ ] kind 필터로 보이는 항목만 "전체 선택" 대상
