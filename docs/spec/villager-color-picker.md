...
- [ ] `randomColor()` — 0~1 범위 RGB 튜플 랜덤 생성
- [ ] "+ 새 villager" 클릭 시 생성 폼 색상을 랜덤으로 초기화
- [ ] 색상 입력을 `<input type="color">` (hex) 로 교체
  - [ ] `rgb01ToHex([r,g,b])` / `hexToRgb01("#rrggbb")` 변환 (0~1 ↔ 0~255 hex)
  - [ ] 현재 0~1 값을 작은 readout 으로 함께 표시 (RON 은 0~1 사용)
- [ ] 편집 폼도 동일 컬러 피커 사용
