...
`@` 멘션 드롭다운에 다른 댓글 작성자 이름을 표시하지 않는다.
`@enji` 만 표시 (로그인 사용자 한정).

### 변경 이유
- 타 댓글 작성자를 `@`로 태그해도 아무 동작이 없음 (라우팅 없음)
- UI 혼란 방지, AI 비서 기능에 집중

### 변경 파일
- `comments.section.tsx`: mentions prop을 `session ? ['enji'] : []` 로 단순화
