...
- **게시글 버전 관리**: 수정 시 이전 버전을 `PostRevision` 컬렉션에 저장. `version` 필드로 이력 추적.
...
- **태그 검색**: 대소문자 무시 정규식(`$regex`, `$options: 'i'`) 기반. 태그 클라우드 폰트 크기는 1.1~2.15rem 범위로 정규화.
