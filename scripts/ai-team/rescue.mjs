...
const 판정풀이 = {
  ...
  NO_TESTS: '**테스트가 하나도 만들어지지 않았습니다.** 스펙이 모호하거나 범위가 너무 넓어 '
    + '무엇을 시험해야 할지 정하지 못한 경우입니다 — 스펙을 좁혀 다시 요청하세요.',
  COLLAPSED_DIR: '**git 이 새 디렉터리를 한 줄로 접어 안의 파일이 안 보였습니다.** '
    + '파이프라인이 디렉터리 경로 하나만 받아 테스트로 못 친 것입니다 — 스펙이 아니라 '
    + '`--untracked-files=all` 같은 git 옵션 차이입니다. 워크트리에서 디렉터리 안을 펼쳐 '
    + '확인해 주세요.',
};
