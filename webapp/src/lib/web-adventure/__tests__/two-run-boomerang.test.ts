// #272 the cross-run boomerang, integration e2e - simulating a two-run sequence.
//
// The flow verified:
//   run 1 - Kael completes Harmony (still_the_engine succeeds).
//     ending → past_runs.push({endingId:"harmony"}).
//   run 2 - starting as Kael (or Rin).
//     buildWorldFlags(past_runs) = {"world.harmony_kept": true}.
//     injected into character.flags.
//     On reaching climax_revolution_path the hidden conditional `echo_of_harmony` is
//     *unlocked* - to: climax_harmony_path.
//   The result: run 2's branch tree differs *because of run 1's outcome*.
...
describe("회차 부메랑 통합 e2e (#272)", () => {
  test("회차 1 Harmony → 회차 2 에서 echo_of_harmony hidden 분기 해금", () => {
    if (!loaded) return;
    // 1) run 1 - Kael completes Harmony.
    ...
    // 3) run 2 - a fresh Kael start with the flags injected.
    //    On reaching climax_revolution_path, choosing the unlocked echo_of_harmony leads to
    //    climax_harmony_path -> ending_harmony.
    ...
    if (s2.phase === "ended") {
      ...
      // Run 1's boomerang is present in run 2's character flags.
  ...
  test("회차 1 Harmony 부재 → 회차 2 에서 echo_of_harmony 분기 미해금", () => {
    if (!loaded) return;
    // Without the flags injected, choosing echo_of_harmony at climax_revolution_path changes nothing.
    let s = startState("kael", loaded, {}); // empty flags.
    const path: string[] = [
      "grab_scalpel",
      "continue",        // #345 — kael_corridor_blade → kael_corridor.
      "to_cargo_dock",
    ...
    // Currently at climax_revolution_path. Trying echo_of_harmony -> the branch is blocked.
    const before = s;
    const tried = gameReducer(s, { type: "MAKE_CHOICE", choiceId: "echo_of_harmony", rng: RNG }, loaded);
    expect(tried).toEqual(before); // unchanged - the conditional flag blocks it.

    // Instead join_revolution -> ending_revolution proceeds normally.
