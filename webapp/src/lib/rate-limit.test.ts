...
describe("rateLimit — 슬라이딩 윈도우", () => {
  ...
  it("limit 회까지 허용, 초과분은 차단", () => {
    ...
    expect(rateLimit("k", 3, 1000, 0)).toBe(false); // the 4th is blocked
  });

  it("윈도우가 지나면 다시 허용", () => {
    for (let i = 0; i < 3; i++) rateLimit("k", 3, 1000, 0);
    expect(rateLimit("k", 3, 1000, 999)).toBe(false); // still inside the window
    expect(rateLimit("k", 3, 1000, 1001)).toBe(true); // the window has passed -> allowed
  });

  it("키가 다르면 독립적으로 카운트", () => {
    expect(rateLimit("a", 1, 1000, 0)).toBe(true);
    expect(rateLimit("a", 1, 1000, 0)).toBe(false);
    expect(rateLimit("b", 1, 1000, 0)).toBe(true); // another key is unaffected
  });
});
...
describe("clientIp — 스푸핑 불가해야 한다 (#386)", () => {
  ...
  it("X-Real-IP 를 먼저 쓴다 — nginx 가 $remote_addr 로 세팅(덮어씀)하는 위조 불가 값", () => {
    // X-Real-IP wins even if the attacker forges XFF.
  ...
  it("XFF 폴백은 **마지막** 항목 — nginx 가 $proxy_add_x_forwarded_for 로 실제 IP 를 뒤에 붙인다", () => {
    // Without X-Real-IP (a direct connection, say) it still does not use the first entry (the attacker's value).
  ...
  it("공격자가 XFF 첫 항목을 아무리 바꿔도 반환값은 실제 IP 로 고정된다", () => {
    ...
    expect(a).toBe(real);
    expect(b).toBe(real);
    expect(a).toBe(b); // the same bucket - the rate limit is not bypassed
  });
