// It loads the deployed file as is (#112).
// The file player.html imports and the file tested here must be **one and the same**, so there is never a case of
// "the test passes but the browser behaves differently".
...
// ── IPS building helpers ────────────────────────────────────────────────────────
/** An ordinary record: offset(3) size(2) data */
...
/** An RLE record: offset(3) 0x0000 count(2) value(1) */
...
describe('retro/rom-patch', () => {
  ...
  describe('IPS', () => {
    ...
    describe('512 헤더 처리', () => {
      ...
      it('stripHeader=false 면 헤더를 포함한 오프셋으로 패치한다', () => {
        ...
        expect(out.rom[10]).toBe(0x22);
        // The same position in the body is unchanged - meaning it patched the position 512 further along
  ...
  describe('crc32', () => {
    it('알려진 값과 맞는다', () => {
      // The CRC-32 of "123456789" is 0xCBF43926 (the standard check vector)
      expect(crc32(u8(enc('123456789'))) >>> 0).toBe(0xcbf43926);
    });
  });

  describe('BPS', () => {
    // BPS: "BPS1" + varint(sourceSize, targetSize, metaSize) + actions + CRC(source, target, patch)
    ...
    /** The minimal BPS that copies the whole source then appends the tail bytes. */
    function makeBps(source: Uint8Array, tail: number[]): Uint8Array {
      ...
      const actions = [
        ...
        // TargetRead: (length-1)<<2 | 1, followed by the data
    ...
    // Made a multiple of 1KB like a real SFC ROM - a size at which the header rule (size % 1024 === 512) holds.
    const source = (() => {
    ...
    it('헤더 있는 롬이어도 CRC 로 맞는 해석을 자동으로 고른다', () => {
      ...
      // It strips the 512 and lines up on its own, without stripHeader being given.
