/**
 * Saju (the four pillars) calculation - a wrapper over lunar-javascript (#390). **Server only** (never bundle for the client).
 *
 * Verified by measurement: the ipchun year-pillar boundary (2000-01-15 -> gi-myo / 2000-02-10 -> gyeong-jin) and the
 * hour pillar are both correct. The element mapping uses **a standard table** directly rather than parsing the
 * library's strings (avoiding renames and parsing risk).
 *
 * Limits: it uses the KST wall clock as is - it applies neither traditional saju's apparent solar time (about -30
 * minutes by longitude) nor the DST some years had (1948-88). A birth near an hour-pillar boundary (around midnight)
 * can be off by one (which the UI states).
 */
...
export interface Pillar {
  ganzhi: string; // the ganji, e.g. "戊午"
  gan: string; zhi: string; // hanja
  ganKr: string; zhiKr: string; // Hangul, e.g. "무", "오"
  ganEl: WuXing; zhiEl: WuXing;
}

export interface Saju {
  pillars: { year: Pillar; month: Pillar; day: Pillar; time: Pillar | null };
  dayGan: string; // the day stem's hanja
  dayGanKr: string; // the day stem in Hangul
  dayEl: WuXing; // the day stem's element = one's own energy
  elements: Record<WuXing, number>; // the distribution of elements
}
...
/**
 * Computes the four pillars from a birth date (and optionally a birth hour).
 * birthday is a Date storing 'YYYY-MM-DD' at UTC midnight, so the year, month and day are read with UTC getters (the #326 convention).
 * Without a birth hour it computes at noon and **the hour pillar is null** (the three date-based pillars do not depend on the time).
 */
...
/** Today's (KST) day pillar - the day's energy. */
...
// The generating cycle: wood -> fire -> earth -> metal -> water -> wood. The overcoming cycle: wood over earth, earth over water, water over fire, fire over metal, metal over wood.
...
/** The ten-god group relationship between one's own element (mine) and another (other), plus a plain-language meaning. */
...
// ── The day's saju reading (LLM plus the politeness guard, shared with reading.ts) ──────────────

/** Relationship key -> a plain one-liner (shared by the template and the prompt). */
...
/** Assembles the prompt (pure). It enforces polite speech with examples (the same tone as tarot). */
...
/** The fallback when the LLM fails or has not run (always polite). */
...
/** Generates the day's saju reading (LLM -> politeness guard -> template). */
...
/** Assembles the stored saju facts into a SajuContext (pure) - shared by the API and the batch. */
...
// ── The client DTO block (assembled on the server) ──────────────────────────────
...
/** Saju facts plus the cached reading -> the client block (pure). With no reading it fills in the template at once. */
