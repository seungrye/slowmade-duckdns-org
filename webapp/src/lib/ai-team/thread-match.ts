 ...
 *
 * **닫는 주체가 바뀌었다 (#222).** 처음엔 사람만 닫게 했다 — AI 가 성급하게 닫을까 봐서다.
 * 주인이 그 권한을 AI 에 위임하기로 정했다. 위임이 안전한 근거는 **되돌리기가 싸다**는 것:
 * 사람이 이 태그를 떼면 스레드가 그대로 다시 열리고, 글도 덧글도 남아 있다.
 * AI 는 `POST /api/ai-team/close` 로 닫는다(그 파일 주석에 제약이 적혀 있다).
 */
