// 〈에테르니아: 정제〉 덱빌딩 — A안(별도 구현)의 타입 (#419).
//
// A안은 침식·판정·엔딩 규칙을 자체 구현한다. **의도적 중복**이다 — B안(엔진 공유)과
// 나란히 놓고 재려는 것이 이 작업의 목적이라, 여기서 web-adventure 를 참조하면 비교가
// 성립하지 않는다.
//
// 다만 **엔딩 id 만은 예외**로 `@/types/web-adventure` 의 것을 쓴다. 나중에 회차를
