// What to accept (#409) - only the judgement. Storing is route.test.ts's concern.
...
describe('parseDiagUpload', () => {
  ...
  it('한글은 바이트로 잰다', () => {
    // Measured in characters, Korean is three times the size and sails past the cap.
    const 한글 = '가'.repeat(MAX_DIAG_BYTES / 3 + 10);
    expect(parseDiagUpload({ body: 한글 })).toBeNull();
  });

  it('버전·기기가 없어도 받는다', () => {
    // An older build might upload it - losing the trail over that is worse.
