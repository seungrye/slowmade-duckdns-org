      ...
      <div className="flex items-center">
        {shown.map((event, i) => (
          <button
            ...
            //
            // 겹침을 28.6%(-ml-2)에서 **89.3%(-ml-[25px])로 올렸다** (#410) — navbar 에서
            // 자리를 너무 먹었다. 스택 폭이 88px → 36px 로 준다. 정확히 90% 면 25.2px 인데
            // 소수 px 는 가장자리를 흐려 정수로 둔다.
            //
            // 대가를 알고 고른 것이다: 첫 칸만 보이고 나머지는 2.8px 실오라기라 **색으로
            // 종류를 알리던 것이 사실상 사라지고**, 배지별 hover 판정도 그만큼 좁아진다.
            // 짚어 보기가 아주 죽지는 않는다 — **누르면 "전부 보기"** 가 되고, 모바일은
            // 원래 hover 가 없어 그 길로만 썼다. 짚어 보기는 데스크톱에서 덤이었다.
