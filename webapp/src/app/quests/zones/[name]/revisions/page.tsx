...
import type { ZoneRevisionDocument } from "@/types/zone";
import { useInfoDialog } from "@/components/info-dialog";

export default function ZoneRevisionsPage() {
  const { name } = useParams<{ name: string }>();
  const decoded = decodeURIComponent(name);
  const [revisions, setRevisions] = useState<ZoneRevisionDocument[]>([]);
  const [loading, setLoading] = useState(true);
  const [restoring, setRestoring] = useState<number | null>(null);
  const { showInfo } = useInfoDialog();

  const load = useCallback(async () => {
    const res = await fetch(`/api/quests/zones/${name}/revisions`);
  ...
  async function restore(version: number) {
    ...
    const res = await fetch(`/api/quests/zones/${name}/revisions/${version}/restore`, {
      method: "POST",
    });
    setRestoring(null);
    if (res.ok) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/quests/zones" className="text-sm text-gray-400 hover:text-gray-600">
          ← 카탈로그
        </Link>
            ...
            <li key={rev._id} className="flex items-center justify-between p-4 border rounded-lg">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  generator: <span className="font-mono">{rev.zone.generator}</span>
                  {rev.zone.description ? ` · ${rev.zone.description}` : ""}
                </p>
              </div>
              <button
