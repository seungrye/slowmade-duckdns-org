...
import type { MonsterRevisionDocument } from "@/types/monster";
import { useInfoDialog } from "@/components/info-dialog";

export default function MonsterRevisionsPage() {
  const { id } = useParams<{ id: string }>();
  const decoded = decodeURIComponent(id);
  const [revisions, setRevisions] = useState<MonsterRevisionDocument[]>([]);
  const [loading, setLoading] = useState(true);
  const [restoring, setRestoring] = useState<number | null>(null);
  const { showInfo } = useInfoDialog();

  const load = useCallback(async () => {
    const res = await fetch(`/api/quests/monsters/${id}/revisions`);
  ...
  async function restore(version: number) {
    ...
    const res = await fetch(`/api/quests/monsters/${id}/revisions/${version}/restore`, {
      method: "POST",
    });
    setRestoring(null);
    if (res.ok) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/quests/monsters" className="text-sm text-gray-400 hover:text-gray-600">
          ← 카탈로그
        </Link>
            ...
            <li key={rev._id} className="flex items-center justify-between p-4 border rounded-lg">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  {rev.monster.displayName}
                  · HP {rev.monster.hp}
                  · ATK {rev.monster.attack}
                  · speed {rev.monster.speed}
                </p>
              </div>
              <button
