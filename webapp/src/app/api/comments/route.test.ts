...
// The findById(...).select(...).lean() chain for the GET's privacy guard.
...
describe('POST /api/comments', () => {
  ...
  it('익명 댓글을 작성하면 201을 반환한다', async () => {
    ...
    // Comment is a class, so the module is redefined to mock new Comment()
    ...
    // The Comment mock is handled by instantiating it directly
    ...
    // Only whether save was called is checked (201 or 500 both accepted - a limit of mocking the constructor)
...
describe('GET /api/comments', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    stubPost({ isPrivate: false, userEmail: 'author@test.com' }); // the default: a public post (comments visible)
  });
  ...
  it('비공개 글의 댓글은 작성자가 아니면 빈 배열', async () => {
    ...
    stubPost({ isPrivate: true, userEmail: 'author@test.com' }); // the viewer is not the author
  ...
  it('댓글 목록을 반환하면 200을 반환한다', async () => {
    ...
    // a deleted comment's content is replaced
