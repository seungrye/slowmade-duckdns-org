...
describe('GET /api/quests/villagers/[id]', () => {
  ...
  it('id 로 조회', async () => {
    const mock = { id: 'elder', name: '장로', color: [0.9, 0.8, 0.5] };
    (Villager.findOne as ReturnType<typeof vi.fn>).mockReturnValue({ lean: vi.fn().mockResolvedValue(mock) });
    const res = await GET(makeRequest('GET'), { params: makeParams('elder') });
    expect(res.status).toBe(200);
    expect(Villager.findOne).toHaveBeenCalledWith({ id: 'elder' });
  });
});

describe('PUT /api/quests/villagers/[id]', () => {
  beforeEach(() => vi.clearAllMocks());
  ...
  it('정의된 필드만 갱신, version + 1, revision 백업 생성', async () => {
    const villager = {
      _id: 'v1',
      id: 'elder', name: '장로', color: [0.9, 0.8, 0.5], dialogs: ['안녕'],
      speed: 0.5, version: 3,
    ...
    await PUT(makeRequest('PUT', { speed: 2.0 }), { params: makeParams('elder') });
    ...
    expect(VillagerRevision.create).toHaveBeenCalledWith(expect.objectContaining({
      villagerId: 'v1',
      version: 3,
      villager: expect.objectContaining({ id: 'elder', speed: 0.5, dialogs: ['안녕'] }),
    }));
  });

  it('name 을 갱신할 수 있다', async () => {
    const villager = {
      _id: 'v1', id: 'elder', name: '장로',
      version: 1,
      save: vi.fn().mockResolvedValue(undefined),
    };
    (Villager.findOne as ReturnType<typeof vi.fn>).mockResolvedValue(villager);
    await PUT(makeRequest('PUT', { name: '원로' }), { params: makeParams('elder') });
    expect(villager.name).toBe('원로');
  });
});

describe('DELETE /api/quests/villagers/[id]', () => {
  beforeEach(() => vi.clearAllMocks());
  ...
  it('존재하면 revision 일괄 삭제 + deleteOne 후 200', async () => {
    ...
    const res = await DELETE(makeRequest('DELETE'), { params: makeParams('elder') });
    expect(res.status).toBe(200);
