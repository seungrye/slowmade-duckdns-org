...
describe('GET /api/posts', () => {
  ...
  it('기본 파라미터로 getPaginatedPosts를 호출한다', async () => {
    ...
    expect(getPaginatedPosts).toHaveBeenCalledWith(1, 9, 'latest', null, true, null);
  });

  it('page, limit, sort 파라미터를 전달한다', async () => {
    (getPaginatedPosts as ReturnType<typeof vi.fn>).mockResolvedValue({ posts: [], total: 0 });
    await GET(makeRequest({ page: '2', limit: '5', sort: 'popular' }));
    expect(getPaginatedPosts).toHaveBeenCalledWith(2, 5, 'popular', null, true, null);
  });

  it('email 파라미터를 전달한다', async () => {
    (getPaginatedPosts as ReturnType<typeof vi.fn>).mockResolvedValue({ posts: [], total: 0 });
    await GET(makeRequest({ email: 'a@test.com' }));
    expect(getPaginatedPosts).toHaveBeenCalledWith(1, 9, 'latest', 'a@test.com', true, null);
  });
  ...
  it('유효하지 않은 sort는 undefined로 처리된다', async () => {
    ...
    expect(getPaginatedPosts).toHaveBeenCalledWith(1, 9, undefined, null, true, null);
  });

  it('limit이 50을 초과하면 50으로 제한된다 (M-2)', async () => {
    (getPaginatedPosts as ReturnType<typeof vi.fn>).mockResolvedValue({ posts: [], total: 0 });
    await GET(makeRequest({ limit: '100000' }));
    expect(getPaginatedPosts).toHaveBeenCalledWith(1, 50, expect.anything(), null, true, null);
  });

  it('page가 0 이하이면 1로 제한된다 (M-2)', async () => {
    (getPaginatedPosts as ReturnType<typeof vi.fn>).mockResolvedValue({ posts: [], total: 0 });
    await GET(makeRequest({ page: '-5' }));
    expect(getPaginatedPosts).toHaveBeenCalledWith(1, 9, expect.anything(), null, true, null);
  });
});
