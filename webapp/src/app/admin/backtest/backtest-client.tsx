...
type FullResult = BacktestResult & { bars: Bar[]; principal: number };
...
export default function BacktestClient() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const cfg = {
        ...
        locPremiumPct: locPremiumPct / 100,
      };
      const r = runBacktest(bars, cfg);
      setResult({ ...r, bars, principal });
    } catch (e) {
  ...
  return (
    <main className="mx-auto px-4 py-8 max-w-5xl">
      <h1 className="text-2xl font-bold mb-1">백테스트 — 무한매수법</h1>
      <p className="text-sm text-gray-500 mb-6">
        과거 일봉으로 라오어 무한매수법을 시뮬레이션합니다. 연산은 브라우저에서 실행돼 서버 부담이 없습니다.
        (시장가=종가, 매수 LOC=저가 터치, 매도=고가 터치 근사 체결. 수수료·슬리피지 미반영.)
      </p>

      {/* 옵션 폼 */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <Field label="종목 코드" hint="예: CB, TQQQ, 069500">
          <input
            value={ticker}
        ...
        <div className="flex items-end">
          <button
            onClick={run}
            disabled={loading}
            className="w-full py-2 rounded bg-blue-600 text-white font-medium hover:bg-blue-700 disabled:opacity-50 transition"
          >
...
function Result({ result }: { result: FullResult }) {
  ...
  const option: EChartsOption = {
    ...
    series: [
      ...
      { name: "매수", type: "scatter", symbol: "triangle", symbolSize: 8, itemStyle: { color: "#dc2626" }, data: buys.map((t) => [t.date, t.price]), z: 5 },
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <Metric label="최대 도달 회차" value={`${maxRound} / ${result.trades.length}건 거래`} />
        ...
        <table className="w-full text-sm border-collapse">
          <thead className="sticky top-0 bg-white dark:bg-gray-900">
            <tr className="border-b text-gray-500 text-left">
              ...
              <th className="py-2 px-3 text-right">회차</th>
            </tr>
          </thead>
          <tbody>
            {result.trades.map((t, i) => (
              <tr key={i} className="border-b border-gray-100 dark:border-gray-800">
                ...
                <td className="py-1.5 px-3 text-right text-gray-500">{t.roundNo}</td>
              </tr>
            ))}
          </tbody>
        </table>
