...
**못 잡는 시험은 없는 것보다 나쁘다** — 있다고 착각하게 만든다. #443 에서 폭 단언 하나가
조상 선택자로 엉뚱한 요소를 재서 **그냥 통과**했다. 그 뒤로 지키는 것들.

### ① 내 스타일 클래스로 요소를 찾지 않는다

특히 Tailwind 유틸리티(`.h-1.overflow-hidden`)는 **디자인이 바뀌면 깨진다** — 기능은
멀쩡한데 시험만 죽는다. 조상 선택자(`ancestor::div[contains(@class,…)]`)는 더 나쁘다.
어느 요소를 잡았는지 눈으로 확인할 수 없어 **틀린 통과**를 만든다.

찾는 순서:

1. `getByRole` + 접근성 이름 — 대개 이게 답이고, 없으면 **만든다**. 만드는 김에 a11y 도
   좋아진다(#443 에서 intent 뱃지에 `aria-label` 을 단 것처럼).
2. `getByText` / `getByLabel` — 사용자가 실제로 읽는 것.
3. 라이브러리가 주는 클래스 훅(`.react-flow__node`) — **대안이 없을 때만.** React Flow 엔
   role 이 없다. 쓸 때는 왜 그것뿐인지 주석을 남긴다.

### ② 대리 지표가 아니라 원하는 것을 잰다

"손패가 판 폭의 95% 이상"은 대리 지표였다. 진짜 원한 것은 **"카드 이름이 보이는가"** 다.
그래서 이웃 카드 중심 간 거리를 재서 `> 카드폭/2` 로 바꿨다 — 구조가 바뀌어도 살아남고,
읽으면 무엇을 지키는지 바로 안다.

### ③ 씨앗을 고정한다

절차 생성이 끼면 회차마다 화면이 달라 개수 단언이 흔들린다(실제로 흔들렸다).
`?seed=42` 로 지도를 못박는다. 시각·난수를 쓰는 코드는 주입형으로 만들어 둔다.

### ④ 고정 sleep 금지 — 조건으로 기다린다

리액트 상태가 반영되기 전에 읽으면 단발 조회는 `-1` 을 준다(실측). `expect.poll` /
`toHaveCount` 같은 **재시도 단언**을 쓴다. `page.waitForTimeout` 은 부하가 걸린 기계에서
반드시 배신한다.

### ⑤ 로케이터에 직접 누른다

`focus()` 후 `keyboard.press()` 사이에도 리렌더가 낄 수 있다 — 전체 실행 3회 중 1회
실패했다. `locator.press()` 는 누르는 순간에 그 요소를 잡는다.

### ⑥ 의심스러우면 일부러 깨 본다

이 시험이 정말 실패할 수 있나? 검증할 코드를 잠깐 틀리게 고쳐 보고 **빨간불이 뜨는지**
확인한다. 순수 모듈(`src/lib/eternia-refine`)은 이걸 자동으로 한다 —
`pnpm test:mutation`(설정 `stryker.config.mjs`, 주 1회 CI). 살아남은 변이 = 빈 단언.

### flake 는 조용히 지나가지 않는다

CI 는 `retries: 2` 라 재시도에 통과하면 잡이 초록이다. 그래서 워크플로가 JSON 보고서를
읽어 **재시도로 통과한 시험을 경고로 띄운다**. 실패로 만들지는 않는다 — 그러면 아무도
안 고치고 재시도를 꺼 버린다. 경고가 뜨면 그 시험은 진짜 경합일 가능성이 높다.
