...
export async function getRevision(revisionId: string): Promise<{ jsonContent: unknown } | null> {
    if (!mongoose.Types.ObjectId.isValid(revisionId)) return null;
    await connectToDB();
    const revision = await PostRevision.findById(revisionId).select('jsonContent').lean() as { jsonContent: unknown } | null;
    if (!revision) return null;
    return { jsonContent: revision.jsonContent };
}
