        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          ...
          {[1, 2, 3].map((id) => (
            <div key={id} className="bg-white rounded-lg shadow-md p-4">
              <Image src={`/humor-${id}.jpg`} alt={`유머 ${id}`} width={300} height={200} className="rounded-md" />
              <h3 className="mt-3 text-lg font-semibold">이 유머가 핫하다!</h3>
              <p className="text-gray-500 text-sm">조회수 1.2K • 댓글 32</p>
              <Link href={`/humor/${id}`} className="text-blue-500 mt-2 block">더 보기 →</Link>
            </div>
          ))}
        </div>
      </section>
